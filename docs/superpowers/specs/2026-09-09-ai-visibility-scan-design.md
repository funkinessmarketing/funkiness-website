...
**Harde eis van Daisy:** het rapport moet iets opleveren dat een bedrijf dat zelf redelijk AI-vaardig is, niet zelf kan namaken. Puur "vraag het aan ChatGPT" is niet genoeg, dat kan iedereen zelf.

**Aangescherpt na kritische zelfreview (2026-09-09):** alleen "wel/niet genoemd" checken is te dun voor een freebie die echt moet knallen. Twee toevoegingen maken het onderscheidend:
1. **Concurrentievergelijking:** niet alleen "jij wordt niet genoemd" maar concreet "ChatGPT noemt [concurrent X en Y] wél bij deze vraag, jou niet." Scherper, persoonlijker, en niet iets wat een ondernemer zelf systematisch voor meerdere concurrenten gaat uitzoeken.
2. **Kant-en-klare deliverable, niet alleen advies:** een pasklaar, geldig JSON-LD schema-blok specifiek voor dit bedrijf, direct te plakken in hun website. Het verschil tussen "hier is wat je mist" en "hier is het al af" is precies wat het onnamaakbaar maakt, want dat vereist code genereren, geen prompt.
...
- **Concurrentievergelijking:** minstens 1 van de 5 vragen is bewust breed geformuleerd, categorie + plaats zonder bedrijfsnaam (bijv. "best hotels in Willemstad Curaçao" in plaats van iets merk-specifieks). Het ruwe antwoord op die vraag gaat mee de rapportgeneratie in (stap 4), waar Claude er de daadwerkelijk genoemde bedrijfsnamen uit haalt (tot 3), zodat het rapport concreet kan zeggen wie er wel genoemd wordt.

### Stap 2b: kant-en-klare schema-snippet (altijd, geen AI nodig)

- Een deterministische functie (geen AI-call, puur templating, dus nooit ongeldige JSON) bouwt een geldig JSON-LD LocalBusiness-blok op basis van de formuliergegevens: bedrijfsnaam, sector (gemapt naar het juiste schema.org-type: hotel→Hotel, restaurant→Restaurant, makelaar→RealEstateAgent, airbnb→LodgingBusiness, anders→LocalBusiness), plaats, en website-URL indien gegeven.
- Dit blok wordt letterlijk als `<code>`-blok in het rapport getoond, met een korte uitleg: plak dit in de `<head>` van je website (of laat je websitebouwer dit doen).
- Dit is exact hetzelfde patroon dat vandaag is toegepast op funkiness.ai zelf (zie `index.html`), dus bewezen werkende aanpak, geen experiment.
...
- `competitors_mentioned`: lijst (max 3) van bedrijfsnamen die de AI wél noemde bij de brede categorie-vraag, leeg als er geen duidelijke namen te herkennen waren
...
De schema-snippet (stap 2b) wordt **niet** door Claude gegenereerd, die komt los uit de deterministische functie en wordt apart in de e-mailtemplate ingevoegd.
...
1. **Naar de aanvrager:** rapport met score-gauge, audit-samenvatting, AI-zoeksamenvatting (incl. genoemde concurrenten indien gevonden), de kant-en-klare schema-code als `<code>`-blok, de 3 actiepunten, de teaser, en een CTA-knop "Plan een gesprek met Daisy" die linkt naar `BOOKING_URL`.
...
- **Geen Make.com-koppeling in deze build.** Daisy wil dit later apart oppakken voor de distributie/opvolging van leads. Bewuste beslissing op 2026-09-09, niet vergeten, niet stilzwijgend laten vallen.
...
8. Sector met duidelijke concurrenten in het gebied, bevestig dat `competitors_mentioned` correct gevuld wordt
9. Controleer dat de gegenereerde schema-snippet geldige JSON-LD is (bijv. met `JSON.parse`) voor elke sector-mapping
